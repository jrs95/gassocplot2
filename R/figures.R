...
plot_recombination_rate <- function(chr, x.min, x.max, build = 37) {
  if (build == 37) {
    recombination.data <- gassocplot2::genetic_map_b37[(gassocplot2::genetic_map_b37$chr == chr & gassocplot2::genetic_map_b37$pos >= x.min & gassocplot2::genetic_map_b37$pos <= x.max), ]
  }
  if (build == 38) {
    recombination.data <- gassocplot2::genetic_map_b38[(gassocplot2::genetic_map_b38$chr == chr & gassocplot2::genetic_map_b38$pos >= x.min & gassocplot2::genetic_map_b38$pos <= x.max), ]
  }
...
plot_gene_two <- function(gene.region, chr, x.min, x.max, stack = FALSE) {
  ...
  plot.genes <- plot.pos + geom_line(data = genes.df.pos, mapping = aes(x = pos, y = y, group = name), colour = "blue4", linewidth = 0.8)
...
plot_gene_fifteen <- function(gene.region, chr, x.min, x.max, stack = FALSE) {
  ...
  plot.genes <- plot.pos + geom_line(data = genes.df.pos, mapping = aes(x = pos, y = y, group = name), colour = "blue4", linewidth = 0.7)
...
plot_assoc <- function(data, corr = NULL, corr.top = NULL, x.min, x.max, top.marker = NULL, ylab, type = "log10p", labels = NULL, sig.thres = NULL, point.padding = 0.15, nudge_x = 0, nudge_y = 0, ylim_prob1 = TRUE) {
  ...
  if (!is.null(sig.thres) & type == "log10p") {
    ylim <- max(ylim, (-log10(sig.thres) + -log10(sig.thres) * 0.1))
  }
  if (type == "prob" & ylim_prob1) {
...
assoc_plot <- function(data, corr = NULL, corr.top = NULL, ylab = NULL, title = NULL, subtitle = NULL, type = "log10p", x.min = NULL, x.max = NULL, top.marker = NULL, legend = TRUE, build = 37, labels = NULL, sig.thres = NULL, point.padding = 0.15, nudge_x = 0, nudge_y = 0, ylim_prob1 = TRUE) {
  ...
  # Genes
  if (build == 37) {
    gene.region <- gassocplot2::genes_b37[(gassocplot2::genes_b37$chr == chr & !(gassocplot2::genes_b37$end < x.min) & !(gassocplot2::genes_b37$start > x.max)), ]
  }
  if (build == 38) {
