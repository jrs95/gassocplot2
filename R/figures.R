...
plot_assoc_stack <- function(data, corr=NULL, corr.top=NULL, x.min, x.max, top.marker=NULL, ylab, type="log10p", highlights=NULL){
  ...
  marker.plot <- marker.plot + theme_bw() +  ylab(ylab) + xlab(NULL) + scale_y_continuous(limits=c(0,ylim)) + scale_x_continuous(limits=c(x.min, x.max), breaks=NULL) 
  ...
  if(geomtext){
    ...
    if(!is.null(highlights)){
      if(hightext){
        ...
        marker.plot <- marker.plot + geom_label_repel(data=highlight_points, aes(x=pos,y=stats,label=marker), nudge_y=(0.1*ylim), size=4.5, point.padding=0.1)
      }
    }
  }else{
   if(!is.null(highlights)){
      if(hightext){
        marker.plot <- marker.plot + geom_label_repel(data=highlight_points, aes(x=label_pos,y=stats,label=marker), size=4.5, point.padding=0.1)
      }else{
        marker.plot <- marker.plot + geom_label_repel(data=lead_marker, aes(x=label_pos,y=stats,label=marker), size=4.5, point.padding=0.1)
      }
    }else{
      marker.plot <- marker.plot + geom_label_repel(data=lead_marker, aes(x=label_pos,y=stats,label=marker), size=4.5, point.padding=0.1)   
   }
